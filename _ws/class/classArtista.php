...
class artista {
	...
	function artista($v = 0){
		...
		$this->id_genero       = ( isset($reg["ID_GENERO"]) ? ( $reg["ID_GENERO"] !== "" ? $reg["ID_GENERO"] : NULL ) : NULL );
		...
		if ($this->id === NULL) {
			...
			$sql = "INSERT INTO ARTISTA(
					...
					VALUES (						
						...
						," . ( $this->id_genero === NULL ? "NULL" : $this->id_genero) . "
			...
			$sql = "UPDATE ARTISTA SET
						...
						,ID_GENERO = " .( $this->id_genero === NULL ? "NULL" : $this->id_genero) . "
