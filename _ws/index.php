...
include ("includes/topo.php");
	...
	if (isset($_GET['marcarComoAtualizado']))
	...
	if ($marcarComoAtualizado == 'artdad') {
	
		$artista = new artista((int)$_GET['id']);
		$artista->flag_ultima_atualizacao_dados = true;
		$result = $artista->save();		
		if ($result) {
			$flag_atualizado = true;
			$msg = 'Dados do artista (' . $artista->nome.') foram "Atualizados!"';
			echo '<span style="color:#fff; background-color:#4B4; text-align:center; padding:5px; width:835px; display:block;">' . $msg . '</span><br><br><br>';
		}
	}
	...
	if ($marcarComoAtualizado == 'cas') {
		...
		if ($marcarComoAtualizado == 'casdad') {
	
		$casa = new casa((int)$_GET['id']);
		$casa->flag_ultima_atualizacao_dados = true;
		$result = $casa->save();		
		if ($result) {
			$flag_atualizado = true;
			$msg = 'Dados da casa (' . $casa->nome.') foram "Atualizados!"';
			echo '<span style="color:#fff; background-color:#4B4; text-align:center; padding:5px; width:835px; display:block;">' . $msg . '</span><br><br><br>';
		}
	}
	...
	if ($marcarComoAtualizado == 'ing') {
		...
		// COLOCA LATITUDE LONGITUDE AÊ!
		/*
		$i = rand(5, 15);
		
		$arrLat = array(
		
			'5' => 'Ow! Coloca uma Latitude aê!?',
			'6' => 'Gosta de mapa? Gosta de mapinha? Então vai lá e pega umas latitudes!', 	
			'7' => 'Olha que tempo lindo! Nada melhor que aproveitar e colocar uma latitudezinha',		
			'8' => 'Virtude! Atitude! Magnitude! Mas nada como uma Latitude!',
			'9' => 'Por favor, coloque umas latitudes! O sistema vai ficar legal!',
			'10' => 'Você é o 1.000.000 visitante! Clique aqui e receba uma viagem para Ibiza com a Jessica Alba ',
			'11' => 'Olha que a rapaziada está sentindo a faaaalta... de um cavaco, de um pandeiro ou de umas latitudes!',
			'12' => 'Latitudes nuas, agorinha mesmo, clique aqui!',
			'13' => '<img src="http://media.carddit.com/a/tSYmjrPZj.gif" />',
			'14' => '<img src="./images/_clique.jpg" />',
			'12' => 'Do Leme ao Pontaaaal... que latitude teráááá?'
		);
		
		
		?>
		<div style="float:left; width: 831px; background-color:#E3CCE1; color:#fff; padding: 5px; text-align:center; font-weight:bold; margin-top:20px;"> 
			<a href="latlon.php">
				<?=$arrLat[$i]?>
			</a>	
		</div>
		...
		*/
		...
		$sql = "
			SELECT 
				...
				DATEDIFF( NOW( ) , ART.ULTIMA_ATUALIZACAO_DADOS ) AS DIAS_SEM_ATUALIZAR_DADOS,
				USU.ID AS USUID, 
				USU.NOME AS USUNOME,
				USU.COR AS USUCOR
			FROM 
		...
		if ($regs["dados"]) {
			...
			foreach ($regs["dados"] as $i) {
				...
				<span style="float:left; padding:7px; background-color:#<?=$cor[($j%2)]?>">
					...
					<a href="artista_cadastro.php?id=<?=$i['ID']?>" style="margin-top:15px; color:#000; float:left; margin-left:5px; display:block; width:220px;"><?=$i['NOME']?> <?=($i['DIAS_SEM_ATUALIZAR_DADOS']>180?"<span style='color:#b44; font-weight:bold'>(D)</span>":"")?></a>
		...
		$sql = "
			SELECT
				...
				DATEDIFF( NOW( ) , CAS.ULTIMA_ATUALIZACAO_DADOS ) AS DIAS_SEM_ATUALIZAR_DADOS,
				CID.ID AS CIDADE_ID,
				CID.NOME AS CIDADE
		...
		if ($regs["dados"]) {
			...
			foreach ($regs["dados"] as $i) {
				...
				<span style="float:left; padding:7px; background-color:#<?=$cor[($j%2)]?>">					
					...
					<a href="casa_cadastro.php?id=<?=$i['CASA_ID']?>" style="margin-top:15px; color:#000; float:left; margin-left:5px; display:block; width:280px;"><?=$i['CIDADE']?> - <?=$i['CASA']?> <?=($i['DIAS_SEM_ATUALIZAR_DADOS']>180?"<span style='color:#b44; font-weight:bold'>(D)</span>":"")?></a>
		...
		if ($regs["dados"]) {
			...
			foreach ($regs["dados"] as $i) {
				...
				<span style="float:left; padding:7px; background-color:#<?=$cor[($j%2)]?>">					
					...
					<a href="ingresso_cadastro.php?id=<?=$i['ID']?>" style="margin-top:15px; color:#000; float:left; margin-left:5px; display:block; width:335px;"><?=$i['NOME']?></a>
		...
		$sql = "
			SELECT 
				...
				ART.ATUALIZAR_DIA AS ATUALIZAR_DIA,
				ART.BOM_INICIO_MES AS BOM_INICIO_MES,				
				DATE_FORMAT( ART.ULTIMA_ATUALIZACAO_DADOS, '%d/%m/%Y' ) AS ULTIMA_ATUALIZACAO_DADOS,
				DATEDIFF( NOW( ) , ART.ULTIMA_ATUALIZACAO_DADOS ) AS DIAS_SEM_ATUALIZAR,
				USU.ID AS USUID, 
				USU.NOME AS USUNOME,
				USU.COR AS USUCOR
			FROM 
			...
			WHERE 
				USU.ID = " . $_SESSION["ID_DADOS"] . " AND
				DATEDIFF( NOW( ) , ART.ULTIMA_ATUALIZACAO_DADOS ) > 180				
			ORDER BY
				DIAS_SEM_ATUALIZAR DESC,
				ART.CATEGORIA DESC,
				ART.NOME ASC	
		...
		if ($regs["dados"]) {
			...
			?>
			<div style="float:left; width: 831px; background-color:#B44; color:#fff; padding: 5px; text-align:center; font-weight:bold; margin-top:20px;">ARTISTAS COM DADOS DESATUALIZADOS (<?=$regs["total"]?>)</div>
			<div style="float:left; font-size:14px; width:1000px; background-color:transparent">
			<?
			...
			foreach ($regs["dados"] as $i) {
				...
				<span style="float:left; padding:7px; background-color:#<?=$cor[($j%2)]?>">
					...
					<div style="float:left; width: 50px;">
						...
						<span style="display:none">
							<a class="btArtRedSmall" href="javascript:void(0)" style="display:block; background-color:#B44; border-radius: 6px; float:left; margin-top:16px; margin-right:10px; width:12px; height:12px;"></a>
							<a href="index.php?marcarComoAtualizado=artdad&id=<?=$i['ID']?>" style="display:block; background-color:#4B4; border-radius: 6px; float:left; margin-top:16px; margin-right:10px; width:12px; height:12px;"></a>
						</span>
					</div>
		...
		$sql = "
			SELECT
				...
				CAS.SITE AS SITE, 
				CAS.BOM_INICIO_MES AS BOM_INICIO_MES,
				DATEDIFF( NOW( ) , CAS.ULTIMA_ATUALIZACAO_DADOS ) AS DIAS_SEM_ATUALIZAR,
				CID.ID AS CIDADE_ID,
				CID.NOME AS CIDADE
			...
			WHERE 
				CID.ID = 5270 AND
				DATEDIFF( NOW( ) , CAS.ULTIMA_ATUALIZACAO_DADOS ) > 180
		...
		if ($regs["dados"]) {
			...
			?>
			<div style="float:left; width: 831px; background-color:#B44; color:#fff; padding: 5px; text-align:center; font-weight:bold; margin-top:20px;">CASAS COM DADOS DESATUALIZADOS (<?=$regs["total"]?>)</div>
			<div style="float:left; font-size:14px; width:1000px; background-color:transparent">
			<?
			...
			foreach ($regs["dados"] as $i) {
				...
				<span style="float:left; padding:7px; background-color:#<?=$cor[($j%2)]?>">					
					...
					<div style="float:left; width: 50px;">
						...
						<span style="display:none">
							<a class="btCasRedSmall" href="javascript:void(0)" style="display:block; background-color:#B44; border-radius: 6px; float:left; margin-top:16px; margin-right:10px; width:12px; height:12px;"></a>
							<a href="index.php?marcarComoAtualizado=casdad&id=<?=$i['CASA_ID']?>" style="display:block; background-color:#4B4; border-radius: 6px; float:left; margin-top:16px; margin-right:10px; width:12px; height:12px;"></a>
						</span>
					</div>
				</span>
				<?		
